...
Esta skill gerencia configurações, diagnósticos e correções de teclado, atalhos e gestos de touchpad no **KDE Plasma 6 (Wayland)**.

## Diagnóstico e Ações

- **Auditoria de Saúde**: Executar `bash shared/check-status.sh` ou `./bin/kde-config status`.
- **Correção de Teclado e Atalhos**: Executar `bash shared/fix-keyboard.sh` ou `./bin/kde-config fix-keyboard`.
